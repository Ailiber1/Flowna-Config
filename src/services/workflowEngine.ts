...
export function generateClaudeCodeInstructions(
  ...
  if (language === 'ja') {
    steps.push(`# ${appName} 開発指示書\n`);
    steps.push(`**Claude Code用の指示書**（Flownaで自動生成）\n`);
    steps.push(`この指示書に従って、以下の作業を順番に実行してください。\n`);

    if (specContent) {
      steps.push(`## 仕様書\n\`\`\`\n${specContent}\n\`\`\`\n`);
    }

    steps.push(`## 実行手順（Claude Codeが自動実行）\n`);
    ...
    steps.push(`## 完了後`);
    steps.push(`デプロイが完了したら、生成されたURLを報告してください。`);

  } else {
    // English version
    steps.push(`# ${appName} Development Instructions\n`);
    steps.push(`**Instructions for Claude Code** (Auto-generated by Flowna)\n`);
    steps.push(`Please execute the following tasks in order.\n`);

    if (specContent) {
      steps.push(`## Specification\n\`\`\`\n${specContent}\n\`\`\`\n`);
    }

    steps.push(`## Execution Steps (Claude Code auto-executes)\n`);
